-- Add 'user_typed' and 'ai_stated' to memory_meta origin CHECK constraint
-- These are valid origins used by the MCP tools and dashboard:
--   user_typed: explicit input in dashboard
--   ai_stated: AI agent explicitly recorded data (e.g., add_record)

-- 1. Update create_user_schema function so NEW user schemas get the wider constraint
...
-- 2. Fix existing user schemas: drop old constraint and add new one
DO $$
DECLARE
  schema_rec RECORD;
BEGIN
  ...
  LOOP
    EXECUTE format(
      'ALTER TABLE %I.memory_meta DROP CONSTRAINT IF EXISTS memory_meta_origin_check',
      schema_rec.schema_name
    );
    EXECUTE format(
      'ALTER TABLE %I.memory_meta ADD CONSTRAINT memory_meta_origin_check CHECK (origin IN (''user_stated'',''user_typed'',''ai_stated'',''ai_inferred'',''ai_pattern'',''imported'',''system''))',
      schema_rec.schema_name
    );
  END LOOP;
END;
